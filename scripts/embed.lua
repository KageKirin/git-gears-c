--
-- Embed the GraphQL queries into src/requests/<api>.c as static data buffers.
-- The queries are embedded at compile time using incbin.
...
--
	...
	function embed(files, queries_c, header)
		...
		local define = "GEARS_" .. symbolicatefile(queries_c):upper() .. "_INC"
		_p("//! git-gears-c's GraphQL queries, as static data buffers.")
	...
	function doembed()
		...
		embed(githubfiles, path.join(script_path,"..", "src", "requests", "github_graphql.h"), true)
		embed(githubfiles, path.join(script_path,"..", "src", "requests", "github_graphql.c"))

		local gitlabfiles = os.matchfiles(path.join(script_path, "..", "src", "requests", "gitlab", "*.graphql"))
		embed(gitlabfiles, path.join(script_path,"..", "src", "requests", "gitlab_graphql.h"), true)
		embed(gitlabfiles, path.join(script_path,"..", "src", "requests", "gitlab_graphql.c"))
	end
